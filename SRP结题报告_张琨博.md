...
我负责本项目的后端部分，开发了一套基于YOLOv8深度学习框架的水稻病害检测REST API系统，能够自动识别和分类水稻的11种主要病害类型。该系统具有高精度的检测能力、完整的API接口体系、容器化部署方案，以及友好的用户集成接口。项目经过143轮迭代训练，最终模型在验证集上获得了良好的性能指标。本报告详细阐述了项目的研究背景、后端采用的技术方案、实现过程、功能特性与部署方案。
...
1. **提升检测效率：** 实现毫秒级病害识别，通过API接口支持单张及批量处理
...
- 数据集来源：https://aidata.nchu.edu.tw/smarter/en/dataset/smarter_04_r14088_0_rice_20230118_img1_123456

  包含水稻的十一种病害与二十七种虫害，有3581张图片与对应标注，总大小20G，免费，下载需VPN。
...
- 支持批量处理，tqdm进度条显示转换状态
- 自动映射11种水稻病害的类别ID(数据集提供了编号映射文件)
- 容错处理：忽略未定义类别的对象
...
# 正常训练（可按需提升轮数）
python train.py --epochs 150 --workers 0 --device cuda
...
# 若续训报 C3k2 兼容错误，先确定ultralytics版本
python -m pip install ultralytics==8.0.196
```

**训练指标分析：** (基于runs/detect/train7/results.csv)

| 指标 | 最佳值 | 说明 |
|------|--------|------|
| **mAP50(B)** | 0.70091 | 50% IoU阈值下的平均精度 |
| **mAP50-95(B)** | 0.41354 | 多IoU阈值(50%-95%)下的平均精度 |
| **精确率(Precision)** | 0.7347 | 检测为正例中实际为正例的比例 |
| **召回率(Recall)** | 0.66867 | 实际正例中被成功检测的比例 |
| **训练收敛** | 第143轮 | 在第143轮达到最优性能 |
...
- 正则化混淆矩阵：<img src="./runs/detect/train7/confusion_matrix_normalized.png" alt="confusion_matrix_normalized" style="zoom:29%;" />

- F1曲线（F1分数随阈值变化曲线）：<img src="./runs/detect/train7/F1_curve.png" alt="F1_curve" style="zoom:29%;" />

- 训练中各项指标随轮数的变化：

<img src="./runs/detect/train7/results.png" alt="results" style="zoom:36%;" />

- 精确率、召回率分别随阈值变化的曲线（P_curve/R_curve):

<img src="./runs/detect/train7/P_curve.png" alt="P_curve" style="zoom:20%;" /><img src="./runs/detect/train7/R_curve.png" alt="R_curve" style="zoom:20%;" />
...
```
***为了方便推理，最终模型已存储至final_models文件夹***
...
### 7.1 模型性能指标（2026年3月最新）

**验证集性能 (best.pt)：**

```
mAP50@IoU=0.50: 0.6154  (61.54%)
mAP50-95@IoU=0.50:0.95: 0.3572  (35.72%)
...
**最新指标解读：**
- **mAP50提升至61.5%：** 检测精度进一步提升，模型泛化能力增强
- **Precision 78%：** 误报率进一步降低，实用性更强
- **Recall 55%：** 漏检率略有波动，建议后续继续优化召回
- **F1-Score 0.644：** 综合表现优良，适合实际部署
- **收敛轮次65：** 训练过程平稳，未出现过拟合

> 注：本节数据基于2026年3月最新训练结果（results.csv），如需复现请参考train.py与数据集配置说明。
